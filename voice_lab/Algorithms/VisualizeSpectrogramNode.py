...
class VisualizeSpectrogramNode(VoicelabNode):
    ...
    def process(self, signal, sampling_rate) -> dict[str: plt.Figure]:
        ...
        voice: parselmouth.Sound = parselmouth.Sound(signal.flatten(), sampling_rate)
        print(signal, sampling_rate)
        print(voice)
        window_length: float = 0.01
        colour_map: Union[str, tuple] = 'afmhot'
        plot_intensity: bool = True
        plot_formants: bool = True
        ...
        spectrogram: parselmouth.Spectrogram = pre_emphasized_voice.to_spectrogram(
            window_length=0.001, maximum_frequency=6000
        )
        x: np.ndarray
        y: np.ndarray
